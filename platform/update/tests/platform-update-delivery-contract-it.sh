...
TARGET_NOTES=Temporary delivery-contract integration target.
EOF
...
output="$(run_update generate "${SOURCE_TARGET_NAME}" --profile tooling)"
...
[[ "${generated_id}" == "${expected_number}_springmaster_platform_update_tooling_for_${SOURCE_TARGET_NAME}" ]] || \
...
with zipfile.ZipFile(zip_path) as zf:
    ...
    assert manifest['requires']['profile']=='tooling'
...
printf '\n# dirty target\n' >> "${TARGET_COPY}/${mutation_path}"
if run_update generate "${SOURCE_TARGET_NAME}" --profile tooling --dry-run >"${WORK_ROOT}/dirty-negative.log" 2>&1; then
...
if run_update generate "${SOURCE_TARGET_NAME}" --profile tooling --dry-run >"${WORK_ROOT}/profile-negative.log" 2>&1; then
