...
class TeamMembershipControllerIntegrationTest {
    @Autowired
    private MockMvc mockMvc;

    @Test
    void qualifiesTargetAwareRelationCandidateAndMutationContracts() throws Exception {
        mockMvc.perform(get("/api/demo/teams/alpha/members")
                        .header("X-Demo-Actor", "team-lead-alpha"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].membershipId", is("membership-001")));

        mockMvc.perform(get("/api/demo/teams/alpha/members/options")
                        .header("X-Demo-Actor", "team-lead-alpha"))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(delete("/api/demo/teams/alpha/members/person-sam")
                        .header("X-Demo-Actor", "admin"))
                .andExpect(status().isNoContent());
    }

    @Test
    void profilesOnlyExplicitlyRegisteredOpenApiOperations() throws Exception {
        mockMvc.perform(get("/api-docs"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.paths['/api/demo/teams/{teamId}/members'].get"
                        + "['x-cocondo-operation-profile'].operationKey", is("demo.teams.members.list")))
                .andExpect(jsonPath("$.paths['/api/demo/catalog/items'].get"
                        + "['x-cocondo-operation-profile']").doesNotExist());
    }
}
