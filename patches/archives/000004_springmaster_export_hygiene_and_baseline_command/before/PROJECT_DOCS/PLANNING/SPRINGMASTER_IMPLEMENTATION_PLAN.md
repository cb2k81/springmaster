...
Nach Patch `000002` ist zusätzlich vorgesehen:
...
1. Exporttool vollständig generalisieren.
2. Patchsystem-Scope-Modell für `springmaster` vervollständigen.
3. Buildtool lokal-first stabilisieren.
4. DBTool von Bootstrap-Status zu vollständiger Liquibase-/MariaDB-Steuerung ausbauen.
5. `.idea/**` und vergleichbare lokale Artefakte konsistent aus Git/Export ausschließen.
...
Nach erfolgreichem Patch `000002` folgt die Tooling-Baseline:

```text
000003_springmaster_tooling_baseline_from_idm.zip
```

Der Patch muss zuerst das Scope-Modell des Patchsystems und die Exportprofile so erweitern, dass spätere Plattform-, Template- und Tooling-Patches ohne Umgehung des Patchsystems möglich sind.
