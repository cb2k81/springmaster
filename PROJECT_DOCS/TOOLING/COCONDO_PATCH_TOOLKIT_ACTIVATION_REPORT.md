...
Die kanonische aktuelle Cocondo-Patch-Toolkit-Version und ihr Runtime-Digest werden ausschließlich aus dem Activation Contract und dem dazu konsistenten Tooling Lock aufgelöst. Git bleibt die dauerhafte Repositoryhistorie. `bin/cpatch` erzeugt, qualifiziert und transferiert neue Patchartefakte aus gebundenen Git-Worktrees. Die akzeptierte `1.1.2`-Baseline und ihre Split-Staging-Korrektur bleiben in Abschnitt 10 historische Evidence; spätere kompatible Runtime-Korrekturen ändern diesen Provenienznachweis nicht.
...
## 14. Toolkit 1.1.3 accept-qualification compatibility hotfix candidate

The failed `000208` accept run `run-20260808T185737Z-d6cd351a7ae9` passed the configured `syntax`, `targeted` and `full` validators and failed only in `qualification-commit`. The Toolkit `1.1.2` method `GitRepository.show_check` invoked `git show --check --oneline --no-patch <commit>`. The qualified DEV-host Git rejects `--check` together with `--no-patch`, so an otherwise valid qualified child commit could not complete its post-commit whitespace check.

Patch `000209_springmaster_patch_toolkit_accept_qualification_compatibility_hotfix` is a compatible patch-level Tooling correction. It changes only that commit-check invocation to `git show --check --oneline <commit>`, retains the existing `--check` semantics, adds a regression fixture, and advances Toolkit `1.1.2` to `1.1.3`. The Tooling component advances from `0.13.0` to `0.13.1` and `PLATFORM_STATE_PATCH` advances to `000209_springmaster_patch_toolkit_accept_qualification_compatibility_hotfix`; aggregate Platform `0.23.0-foundation` and Maven `0.23.0-foundation-SNAPSHOT` remain unchanged because this is an in-sprint compatible Tooling correction rather than a release-closing aggregate version change.

Temporary Runtime qualification `STEP_05V3` reproduced the failing Git semantics on the actual DEV host, proved the corrected command, validated the corrected CLI and artifact inspection, and left repository state unchanged. Because the installed `1.1.2` runtime itself is defective in the accept-finalization path, the separate self-update accept requires the documented bootstrap pointer: a qualified temporary `1.1.2` compatibility runtime may start only this Toolkit self-update accept; observation switches immediately back to canonical `process-ops` once the new commit is visible. This section does not claim acceptance; canonical dry-run and the separate explicit bootstrap accept remain mandatory.

The activation contract now separates the immutable accepted `000203_springmaster_codex_cutover_foundation` identity from the current `versionClosure`. `codexCutoverFoundationAcceptance` retains the accepted 000203 patch/delivery identity, Platform `0.23.0-foundation`, Tooling `0.13.0` and accepted commit `93ab563cc1e82bc801907399602fe04e6d37e2f7`; the current version closure independently advances to Platform `0.24.0-foundation`, Tooling `0.14.0`, Maven `0.24.0-foundation-SNAPSHOT` and State Patch `000209`. The Toolkit runtime correction itself remains patch-level (`1.1.2` to `1.1.3`), while the Springmaster activation-contract extension is minor. The activation validator and its negative fixture enforce this separation so later compatible Tooling changes cannot rewrite historical cutover acceptance evidence.

## 15. Toolkit 1.1.4 Python runtime portability contract

The canonical Toolkit runtime remains launched through `bin/cocondo-toolkit-launcher.sh`, but the launcher now makes its Python compatibility boundary explicit: it resolves `python3` from the active operator environment and fails closed below Python 3.10. The runtime is qualified on the DEV system Python 3.10 line and on the separately qualified Python 3.12 bootstrap line. Python 3.12 is therefore a self-update bootstrap and compatibility oracle, not a permanent product dependency.

Toolkit `1.1.4` corrects both Python-3.10-incompatible nested same-quote f-string expression occurrences in `cocondo_toolkit/patching.py` without changing Patch Manifest V5 or patch transaction semantics. Qualification compiles every packaged Python member under both runtime lines and exercises `ctool-doctor`, `cpatch doctor`, workspace routing, activation integration, the broad Tooling selfcheck, project validator profiles and Maven verification.

The compatible correction advances `PLATFORM_TOOLING_VERSION` to `0.14.1` and `PLATFORM_STATE_PATCH` to `000219_patch-toolkit-python310-portability`. Aggregate Platform `0.24.0-foundation` and Maven `0.24.0-foundation-SNAPSHOT` remain unchanged. Historical `000203` and `000209` acceptance evidence remains immutable.

## 16. Toolkit 1.1.5 staged-path rename parity correction

The failed canonical dry-run `run-20260814T184824Z-391d72ebf4ce` exposed an inventory defect after the Toolkit had correctly staged the manifest paths. Toolkit `1.1.4` implemented `GitRepository.staged_paths` with `git diff --cached --name-only -z`. For a tracked deletion and a sufficiently similar addition, ordinary Git rename detection can represent the pair as `R<score>`; in that representation `--name-only` emits only the destination. The resulting path projection omitted the deleted manifest path and caused `GIT_STAGED_PATH_PARITY_FAILED` even though the index contained the required deletion and addition.

Toolkit `1.1.5` makes the staged inventory explicit with `git diff --cached --no-renames --name-only -z`. It does not change staging, manifest contents, or the equality check: the effective staged-path set must still equal the Patch Manifest path set exactly, and missing or unexpected paths remain blocking. Regression `PATCH_TOOLKIT_STAGED_PATH_RENAME_PARITY_REGRESSION_V1` proves the original `R100` false mismatch, the corrected delete/add parity, and fail-closed rejection of an unexpected staged path.

The compatible patch correction advances `PLATFORM_TOOLING_VERSION` to `0.14.2` and `PLATFORM_STATE_PATCH` to `000222_patch-toolkit-staged-path-parity`. Aggregate Platform `0.24.0-foundation`, Maven `0.24.0-foundation-SNAPSHOT`, and all other component versions remain unchanged. This source state does not claim successful dry-run, acceptance, integration, or repair of `000221_sprint2-closure-amend-002`.

## 17. Post-Sprint-004 activation version-closure correction

Sprint 005 A001 exposed a pre-existing accepted-baseline drift during trusted qualification. The A001 implementation itself passed postcheck, its targeted autonomous-run integration test, and every other independently executed qualification oracle. `tooling-selfcheck.sh` failed only because the current Activation Contract still described the Sprint-003 closure (`0.25.0-foundation`, Tooling `0.15.0`, State Patch `000237_sprint3-autonomous-dod-recovery`) while the accepted repository already carried Platform `0.26.0-foundation` and State Patch `000249_s004-trusted-closure`. The A001 scope did not authorize this contract, so the failure is classified `SCOPE_EXPANSION_REQUIRED` with root cause `POST_ACCEPT_VERSION_CLOSURE_DRIFT`; A001 is not reinvoked.

Patch `000251_post-s004-version-closure-activation-hotfix` is the bounded compatible Tooling correction. Aggregate Platform remains `0.26.0-foundation` and Maven remains `0.26.0-foundation-SNAPSHOT`; `PLATFORM_TOOLING_VERSION` advances from `0.15.0` to `0.15.1` and `PLATFORM_STATE_PATCH` advances to `000251_post-s004-version-closure-activation-hotfix`. The Activation Contract is synchronized to those values. Historical Sprint-003, Codex-cutover and earlier Toolkit acceptance evidence remains unchanged.

The regression is added to `PlatformVersionPropertiesTest`: every Maven test/full/release execution now compares the live Activation Contract `versionClosure` with `platform/versions/platform.env`, including Platform, Tooling, State Patch and Maven Snapshot. A future release-closing change that updates version truth without the Activation Contract therefore fails in the Maven qualification path even when Tooling is not part of the patch scope. This correction does not modify the Toolkit runtime, `cpatch`, `process-ops`, agent-task safety primitives or automatic acceptance policy.

## 18. Sprint-005 trusted closure candidate

Sprint 005 qualifies the autonomous Logical-Run repair orchestration without changing Cocondo Patch Toolkit `1.1.5` transaction or acceptance semantics. The final accepted S005 product baseline is `ab13913c9cdd221b80c11530c72e9b8632cf6705`; the real Self-Use Canary reaches `PREACCEPT` through candidate-local workspace/create, integration-side inspect/plan and the canonical `process-ops` patch dry-run while automatic accept remains false.
