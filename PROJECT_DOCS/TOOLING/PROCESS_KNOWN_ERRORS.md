...
**Cause:** an unknown top-level file, link, special file, unreadable record, unsupported noncanonical patch ID or conflicting typed identity exists. Six-digit text is never a fallback identity source. A numeric-only legacy run is accepted only when `cocondo.run-record.v1`, an allowed patch command, the UUID artifact identity and the canonical patch ID embedded in `metadata.artifactFile` agree exactly. One canonical acceptance record may own the number and demote only differently named terminal failed run attempts to `IGNORE_AND_COUNT`. A historical noncanonical patch ID that never claimed a six-digit identity is compatible only when one matching `cocondo.patch-acceptance.v2` record, the same canonical artifact ID, a successful dry-run and a successful accept agree exactly; it is retained as `IGNORE_AND_COUNT` and reserves no number. Delivery conflicts, unsupported noncanonical runs, unresolved acceptance records and multiple accepted owners remain blocking.

**Recovery:** preserve the entry as evidence and classify or correct its producing workflow. For a historical numeric-only run, verify the immutable run record and artifact filename rather than rewriting either. For a historically accepted noncanonical identity, verify the immutable acceptance, dry-run and successful accept records; never invent or backfill a six-digit number. Do not rename historical records merely to obtain the desired next number and do not provide a patch ID manually. `DELIVERY_LEGACY_ACCEPTED_PATCH_UNVERIFIED` or `DELIVERY_LEGACY_ACCEPTED_PATCH_CONFLICT` means that this evidence chain is incomplete or inconsistent and must remain blocking.
