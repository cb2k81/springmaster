...
- The host harness separately validates the complete `codex exec --json` stream before it reports a governed invocation as successful. Non-JSON lines, Codex `error` items, `turn.failed`, incomplete command executions or non-zero completed command exits are blocking. Implementation mode additionally requires at least one successfully completed `command_execution`. The raw Codex process exit code remains recorded as a fact and is not rewritten to hide a semantic JSONL failure.
...
- Codex shell network access is disabled during the pilot. The Codex parent process may reach only its control plane through the outer host sandbox; host inspection must preflight DNS and HTTPS without granting network access to agent shell commands. If host resolver state is backed by `/run`, only the resolver file content may be copied into private scratch and re-exposed read-only at its original sandbox path; the host `/run` tree remains hidden. Network-dependent dependency installation is a separate human-controlled preparation step and is not part of an agent task.
