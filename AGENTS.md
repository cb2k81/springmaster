...
- `patches/work/` ist der kurzlebige, projektlokale Operator-Handoff-Workspace. Vor jedem Writer (`patch-dry-run`, `patch-accept`, `diagnose`, `incident`, `diagnostic-handoff`, `delivery-prepare`) startet `process-ops` denselben fail-closed Workspace-Lifecycle. Der Root muss bereits existieren; Observer bereinigen ihn nie.
- Diagnoseartefakte für einen Chat-/Operator-Handoff werden ausschließlich über `process-ops diagnostic-handoff` als genau ein ZIP unter `patches/work/` erzeugt. Der nächste Writer entfernt auch diesen Handoff. Codex darf den Workspace weder lesen noch beschreiben.
- Ein konfigurierter Artefakt-Root ist noch keine Autorisierung. Vor relativer Nutzung muss `process-ops artifact-root-authorize` einen exakten Record im Git-Common-Prozesszustand erzeugt haben; Home, Downloads, Repository, Git-Common-Verzeichnis und System-Temp sind als Ziele gesperrt.
- Delivery- und Patch-IDs werden nur über `process-ops delivery-inventory|delivery-next-id|delivery-prepare` aus typisierten Records bestimmt. Namen, UUIDs, Freitext und beliebige sechsstellige Zahlen reservieren keine Identität.
...
- Worktree-, Run- und Artefakt-Roots werden ueber `COCONDO_WORKTREE_ROOT`, `COCONDO_AGENT_RUN_ROOT` und `COCONDO_ARTIFACT_ROOT` absolut und ausserhalb des Repositories konfiguriert. Sie muessen vorher durch den Operator angelegt worden sein; der Harness erzeugt sie nicht stillschweigend.
- Generierte Hostkommandos mit Seiteneffekten benoetigen vor Ausfuehrung eine Wirkungserklaerung nach `PROJECT_DOCS/TOOLING/OPERATOR_COMMAND_EFFECT_CONTRACT.md`. Eine manuelle Ausfuehrung autorisiert keine undokumentierten Writes, Verzeichnisanlagen oder Overwrites.
- Der Harness `bin/agent-task.sh` ruft Codex nicht auf und bietet keine Commit-, Merge-, Cherry-pick-, Push- oder Main-Integration an. `record-invocation` uebernimmt nur bereits erzeugte, unter dem externen Artefakt-Root bereitgestellte Effect- und Invocation-Records, prueft sie und bindet sie unveraenderlich an den Run.
- Codex wird nur als nichtinteraktives `codex exec` mit `--ephemeral`, ignorierten Repository-Regeln, JSON-Ausgabe, explizitem Modell und `--ask-for-approval never` nachgewiesen. Der Host-Harness erzeugt im privaten ephemeren `CODEX_HOME` ein modusgebundenes Permission Profile (`:read-only` fuer Analysis/Qualification, `:workspace` fuer Implementation) und sperrt `/run/codex-home/auth.json` fuer Sandbox-Kommandos. `--ignore-user-config`, Legacy-`--sandbox`/`-s`, `--permission-profile`/`-P`, `--add-dir`, Konfigurations-/Profil-Overrides, Full-Auto und Sandbox-Bypass sind verboten.
- Ein governed Codex-Invoke ist nur bei aeusserem Prozess-Exit `0` **und** bestandener JSONL-Semantik erfolgreich: jede nichtleere Zeile muss valides JSON sein, `error`-/`turn.failed`-Ereignisse sind verboten, gestartete Command-Executions muessen abgeschlossen werden und abgeschlossene Commands muessen `status=completed` sowie `exit_code=0` haben. Implementation verlangt mindestens eine erfolgreich abgeschlossene `command_execution`. Ein aeusserer Codex-Exit `0` allein ist niemals ein Erfolgsoracle.
- Fuer Codex sind Operator-Home, Operator-Handoff-/Download-Verzeichnisse, Integrations-Checkout, Git-Common-Verzeichnis, externe Run-/Artefakt-Roots, andere Repositories und Host-Temp-Verzeichnisse immer schreibgeschuetzt. Auch eine Operator-Wirkungserklaerung darf diese Grenze nicht erweitern. Handoff nach Downloads ist eine getrennte vertrauenswuerdige Operator-Aktion nach Taskabschluss.
