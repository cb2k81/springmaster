...
- Codex wird nur als nichtinteraktives `codex exec` mit `--ephemeral`, ignorierter User-Konfiguration und Repository-Regeln, JSON-Ausgabe, explizitem Modell, `--ask-for-approval never` und modusabhaengigem `read-only`- beziehungsweise `workspace-write`-Sandboxprofil nachgewiesen. `--add-dir`, Konfigurations-/Profil-Overrides, Full-Auto und Sandbox-Bypass sind verboten.
